...
var lodash = createCommonjsModule(function(module, exports) {
  (function() {
    ...
    var runInContext = function runInContext2(context) {
      ...
      var setTimeout = ctxSetTimeout || function(func, wait2) {
      ...
      function debounce(func, wait2, options) {
        ...
        function flush2() {
          return timerId === undefined$1 ? result2 : trailingEdge(now4());
        }
        function debounced() {
...
function formatOffset(offset2, format) {
  const hours2 = Math.trunc(Math.abs(offset2 / 60)), minutes = Math.trunc(Math.abs(offset2 % 60)), sign = offset2 >= 0 ? "+" : "-";
  switch (format) {
    case "short":
      return `${sign}${padStart(hours2, 2)}:${padStart(minutes, 2)}`;
    case "narrow":
      return `${sign}${hours2}${minutes > 0 ? `:${minutes}` : ""}`;
    case "techie":
      return `${sign}${padStart(hours2, 2)}${padStart(minutes, 2)}`;
    default:
...
var Info = class {
  ...
  static features() {
    let intl = false, intlTokens = false, zones2 = false, relative = false;
    if (hasIntl()) {
      intl = true;
      intlTokens = hasFormatToParts();
      relative = hasRelative();
      try {
        zones2 = new Intl.DateTimeFormat("en", {timeZone: "America/New_York"}).resolvedOptions().timeZone === "America/New_York";
      } catch (e) {
        zones2 = false;
      }
    }
    return {intl, intlTokens, zones: zones2, relative};
  }
};
...
var DateTime = class {
  ...
  static fromFormat(text2, fmt, opts = {}) {
    ...
    const {locale = null, numberingSystem = null} = opts, localeToUse = Locale.fromOpts({
      ...
      numberingSystem,
      defaultToEN: true
    }), [vals, parsedZone, invalid] = parseFromTokens(localeToUse, text2, fmt);
    if (invalid) {
      return DateTime.invalid(invalid);
    } else {
...
// build/dist/derivedMemo.js
function derivedWithKey(stores, fn, keyFn) {
  const single = !Array.isArray(stores);
  const stores_array = single ? [stores] : stores;
  const initial_value = fn(single ? get_store_value(stores_array[0]) : stores_array.map(get_store_value));
  let key = keyFn(initial_value);
...
var nowMinute = derivedWithKey(now3, ($now) => $now.startOf("minute"), ($nowMinute) => $nowMinute.toISO());
var nowHour = derivedWithKey(nowMinute, ($nowMinute) => $nowMinute.startOf("hour"), ($nowHour) => $nowHour.toISO());
...
var barEndHour = derived([query_store_default, barStartHour], ([$query, $barStartHour]) => Math.max(Number($query.end ?? 19), $barStartHour + 1));
...
var hours = derived([barStartHour, barEndHour], ([$barStartHour, $barEndHour]) => {
  const hours_ = [];
...
var hourGradient = (localDayStart2, zone, backgroundColor, hours2) => {
  const length = hours2.length;
...
function create_fragment(ctx) {
  ...
  return {
    c() {
      ...
      attr(label0, "class", "svelte-qxgdyp");
      attr(select, "id", "localTimeZone");
      attr(select, "class", "svelte-qxgdyp");
      if (ctx[0].tz === void 0)
        add_render_callback(() => ctx[5].call(select));
      attr(label1, "for", "barStartHour");
      attr(label1, "class", "svelte-qxgdyp");
      attr(input0, "id", "barStartHour");
      attr(input0, "type", "number");
      attr(input0, "max", input0_max_value = String(ctx[3] - 1));
      attr(input0, "placeholder", input0_placeholder_value = String(ctx[4]));
      attr(input0, "class", "svelte-qxgdyp");
      attr(label2, "for", "barEndHour");
      attr(label2, "class", "svelte-qxgdyp");
      attr(input1, "id", "barEndHour");
      attr(input1, "type", "number");
      attr(input1, "min", input1_min_value = String(ctx[4] + 1));
      attr(input1, "placeholder", input1_placeholder_value = String(ctx[3]));
      attr(input1, "class", "svelte-qxgdyp");
      attr(div, "class", "config svelte-qxgdyp");
      attr(button, "class", "svelte-qxgdyp");
    },
    m(target, anchor) {
    ...
    p(ctx2, [dirty]) {
      ...
      if (dirty & 8 && input0_max_value !== (input0_max_value = String(ctx2[3] - 1))) {
        attr(input0, "max", input0_max_value);
      }
      if (dirty & 16 && input0_placeholder_value !== (input0_placeholder_value = String(ctx2[4]))) {
      ...
      if (dirty & 16 && input1_min_value !== (input1_min_value = String(ctx2[4] + 1))) {
        attr(input1, "min", input1_min_value);
      }
      if (dirty & 8 && input1_placeholder_value !== (input1_placeholder_value = String(ctx2[3]))) {
...
function instance($$self, $$props, $$invalidate) {
  ...
  component_subscribe($$self, barEndHour, ($$value) => $$invalidate(3, $barEndHour = $$value));
  component_subscribe($$self, barStartHour, ($$value) => $$invalidate(4, $barStartHour = $$value));
...
function create_fragment2(ctx) {
  ...
  <ul><li class="svelte-y8zofw"><a target="_blank" rel="noreferrer noopener" href="https://www.google.com/search?q=how+to+set+timezone+in+windows+10">how to set timezone in windows 10</a></li> 
    <li class="svelte-y8zofw"><a target="_blank" rel="noreferrer noopener" href="https://www.google.com/search?q=how+to+set+timezone+in+outlook">how to set timezone in outlook</a></li> 
...
function instance4($$self, $$props, $$invalidate) {
  ...
  let $localTimeZone;
  let $nowMinute;
  component_subscribe($$self, localTimeZone, ($$value) => $$invalidate(5, $localTimeZone = $$value));
  component_subscribe($$self, nowMinute, ($$value) => $$invalidate(4, $nowMinute = $$value));
  var _a;
  let {zone = $localTimeZone} = $$props;
  let {timeFormat = DateTime.TIME_SIMPLE} = $$props;
  $$self.$$set = ($$new_props) => {
    $$invalidate(6, $$props = assign(assign({}, $$props), exclude_internal_props($$new_props)));
    if ("zone" in $$new_props)
      $$invalidate(1, zone = $$new_props.zone);
    if ("timeFormat" in $$new_props)
      $$invalidate(2, timeFormat = $$new_props.timeFormat);
  };
  $$self.$$.update = () => {
    $:
      $$invalidate(0, timeStr = ($$invalidate(3, _a = $$props.time) !== null && _a !== void 0 ? _a : $nowMinute).setZone(zone).toLocaleString(timeFormat));
  };
  $$props = exclude_internal_props($$props);
  return [timeStr, zone, timeFormat, _a, $nowMinute];
}
...
function create_each_block3(key_1, ctx) {
  ...
  let zonehours;
  let current;
  zonehours = new ZoneHours_svelte_default({
    props: {
      zone: ctx[2],
  ...
  return {
    ...
    m(target, anchor) {
      ...
      append(div, t0);
      insert(target, t1, anchor);
      mount_component(zonehours, target, anchor);
      current = true;
    },
    p(new_ctx, dirty) {
      ctx = new_ctx;
      if ((!current || dirty & 1) && t0_value !== (t0_value = ctx[2] + ""))
        set_data(t0, t0_value);
      if (!current || dirty & 1) {
        set_style(div, "background-color", ctx[3]);
      }
      const zonehours_changes = {};
      if (dirty & 1)
        zonehours_changes.zone = ctx[2];
      if (dirty & 1)
        zonehours_changes.backgroundColor = ctx[3];
      zonehours.$set(zonehours_changes);
    },
    i(local) {
      if (current)
        return;
      transition_in(zonehours.$$.fragment, local);
      current = true;
    },
    o(local) {
      transition_out(zonehours.$$.fragment, local);
      current = false;
    },
    d(detaching) {
      if (detaching)
      ...
      if (detaching)
        detach(t1);
      destroy_component(zonehours, detaching);
    }
  };
}
...
function instance10($$self, $$props, $$invalidate) {
  ...
  $$self.$$.update = () => {
    if ($$self.$$.dirty & 2) {
      $:
        $$invalidate(0, zones_with_colors = $zones.map((zone, i) => [zone, colors_default[i % colors_default.length]]));
    }
  };
  return [zones_with_colors, $zones];
}
